...
When we launch a CUDA kernel, the GPU runs the function in parallel across thousands of **threads**. Each thread executes the same code but works on a different piece of the data.
...
- A **thread** is a single execution unit that runs the kernel function once.
...
That loop runs K times per thread. Every thread does this independently.
...
Without this guard, those threads would write to memory addresses outside the output array.
...
ctypes (which we use on later) looks up functions by their plain name. If the name is mangled, ctypes cannot find it.
